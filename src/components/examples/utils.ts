...
export const simulateServer = async (values: any) => {
  ...
  if (values.username === 'admin' || values.username === 'root' || values.username === 'taken') {
    return [{ path: ['username'], message: 'Username is not available' }];
  }

  if (values.email?.includes('admin')) {
    errors.push({ path: [], message: 'Administrative accounts cannot be created through this form' });
  }
  
  if (values.user?.profile?.name === 'admin') {
    errors.push({ path: ['user', 'profile', 'name'], message: 'Reserved username' });
  }

  if (values.todos?.some((todo: any) => todo.text.includes('bad'))) {
    errors.push({ path: [], message: 'Todo list contains inappropriate content' });
  }
